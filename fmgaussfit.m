function [fitresult, resnorm] = fmgaussfit(xx,yy,zz)
...
% reduced chi-squared
dz = z-zf;
rr = 1./(numel(z)-6).*sum(dz.^2./ze.^2); % minus 8 because there are 7 fit parameters +1 (DOF)
end

function z = gaussian2D(par,xy)
% compute 2D gaussian
z = par(5) + par(1)*exp(-((xy{1}-par(3)).^2+(xy{2}-par(4)).^2)/(2*par(2)^2));
end
